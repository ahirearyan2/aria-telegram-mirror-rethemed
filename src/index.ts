...
setEventCallback(eventRegex.commandsRegex.tarmirror, eventRegex.commandsRegexNoName.tarmirror, async (msg, match) => {
  if (msgTools.isAuthorized(msg) < 0) {
    msgTools.sendUnauthorizedMessage(bot, msg);
  } else {
    try {
...
setEventCallback(eventRegex.commandsRegex.stats, eventRegex.commandsRegexNoName.stats, async (msg) => {
  if (msgTools.isAuthorized(msg) < 0) {
    ...
    try {
      ...
      msgTools.sendMessage(bot, msg, `<b>Bot Uptime</b>: ${botUptime}\n<b>Total Disk Space</b>: ${downloadUtils.formatSize(diskSpace.size)}\n<b>📦Used</b>: ${downloadUtils.formatSize(usedDiskSpace)}  <b>🗃Free</b>: ${downloadUtils.formatSize(diskSpace.free)}\n\n<b>📊CPU Load</b>: ${avgCpuLoad}`);
    } catch (error) {
...
setEventCallback(eventRegex.commandsRegex.Status, eventRegex.commandsRegexNoName.Status, (msg) => {
  if (msgTools.isAuthorized(msg) < 0) {
    msgTools.sendUnauthorizedMessage(bot, msg);
  } else {
...
setEventCallback(eventRegex.commandsRegex.cancel, eventRegex.commandsRegexNoName.cancel, (msg, match) => {
...
setEventCallback(eventRegex.commandsRegex.help, eventRegex.commandsRegexNoName.help, async (msg, match) => {
  if (msgTools.isAuthorized(msg) < 0) {
    ...
    /mirror URL/Torrent : Download from the given URL and upload it to Google Drive

    /unzipmirror url : Unzip the archive and uploads the unzipped folder. Supported filetypes: .zip, .gz, .bz2, .tar, tar.gz, tar.bz2, .tgz, .tbz2

    /tarmirror URL/Torrent : Same as <code>/mirror</code>, but archive multiple files into a tar before uploading it.

    /status : Send a status message about all active and queued downloads.

    /cancel gid : Cancel a particular mirroring task. Send it as a reply to the message that started the download that you want to cancel or with gid.
    ...
    /clone driveUrl : Clone any shareable drive link.

    /count driveUrl : Obtain informations about a drive folder and send it as a table.

    /tar driveUrl : Create a tar of drive folder and upload to drive.

    /getlink driveUrl : Get the corresponding index link.
    ...
    Note: All the above command can also be called using dot(.) instead of slash(/). For e.x: <b>.mirror </b>url </i>
    `
...
function driveUploadCompleteCallback(err: string, gid: string, url: string, filePath: string,
  ...
  if (err) {
    ...
    if (fileSize) {
      ...
      finalMessage = `📂File Size: ${fileSizeStr}\n\n<b>GDrive Link</b>: <a href="${url}">☁️GDRIVE LINK☁️</a>`;
    } else {
      finalMessage = `📂File Size: ${fileSizeStr}\n\n<b>GDrive Link</b>: <a href='${url}'>☁️GDRIVE LINK☁️</a>`;
    }

    if (gdIndexLink && constants.INDEX_DOMAIN) {
      finalMessage += `\n\n<b>INDEX LINK</b>: <a href="${gdIndexLink}">🚀INDEX LINK🚀</a>`;
    }

    if (constants.IS_TEAM_DRIVE && isFolder) {
      finalMessage += '\n\n<i>File or Folder Uploaded Succesfully</i>';
    }
